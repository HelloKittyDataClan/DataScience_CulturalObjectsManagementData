...
class Acquisition(Activity):
    def __init__(self, refersTo_cho: CulturalHeritageObject, institute: str, person: str, start: str, end: str, technique: str, tool: str|set[str]|None):
...
class QueryHandler(Handler):
    ...
    def getById(self, id: str) -> pd.DataFrame:
        ...
        if id.isdigit():
            ...
            SELECT DISTINCT ?object ?id ?type ?title ?date ?owner ?place ?author_name ?author_id ?author
            WHERE {
...
puh.setDbPathOrUrl("relational.db")
...
class BasicMashup(object):
    ...
    def getEntityById(self, related_id: str):
        ...
        for handler in self.metadataQuery:
            ...
            print(tabulate.tabulate(entity_df,headers="keys"))
            ...
            for author in authors:
                author_obj = Person(author["author_name"], author["author_id"])
            ...
            if row["type"] == base_url + "nautical chart":
                new_object = NauticalChart(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "manuscript plate":
                new_object = ManuscriptPlate(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "manuscript volume":
                new_object = ManuscriptVolume(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "printed volume":
                new_object = PrintedVolume(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "printed material":
                new_object = PrintedMaterial(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "herbarium":
                new_object = Herbarium(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "specimen":
                new_object = Specimen(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "painting":
                new_object = Painting(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == base_url + "model":
                new_object = Model(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            elif row["type"] == "map":
                new_object = Map(related_id, row["title"], row["owner"], row["place"], row["date"], list_of_authors)
            else:
                continue  # Passa all'handler successivo se il tipo dell'oggetto non corrisponde a nessuno dei tipi specificati

            return new_object

        #return None  # Restituisce None se nessun handler ha restituito un oggetto valido
...
grp_endpoint = "http://192.168.178.167:9999/blazegraph/"
path = "data/meta.csv"
path_ = "data/process.json"
...
q.setDbPathOrUrl("relational.db")
qq.setDbPathOrUrl(grp_endpoint)
mashup = AdvancedMashup()
mashup.addProcessHandler(q)
mashup.addMetadataHandler(qq)

#print(q.getById("ULAN:500114874"))
#print(mashup.getEntityById("1"))

#act = mashup.getAllActivities()
#for i in act:
    #print(i.start, i.end, i.person, i.tool, i.institute, i.getRefersTo_cho())
#    print(i)

#print(tabulate.tabulate(process_handler.getAllActivities(),headers="keys"))
